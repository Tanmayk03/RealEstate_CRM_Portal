...
const initial = [
  { id: "CN-1001", customer: "Ananya Sharma", amount: "₹5,000", date: "2026-03-28", status: "Applied", reason: "Billing adjustment" },
  { id: "CN-1002", customer: "Vikram Singh", amount: "₹2,500", date: "2026-04-01", status: "Draft", reason: "Late delivery" },
];
...
export default function CreditNotes() {
  ...
  const [form, setForm] = useState({ customer: "", amount: "", date: "", status: "Draft", reason: "" });
  ...
  const save = () => {
    ...
    else setRows((p) => [{ ...form, id: `CN-${2000 + p.length}` }, ...p]);
    setOpen(false);
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          onClick={() => {
            setEdit(null);
            setForm({ customer: "", amount: "", date: "", status: "Draft", reason: "" });
            setErrors({});
            setOpen(true);
          }}
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={edit ? "Edit Credit Note" : "New Credit Note"} size="lg">
        <div className="space-y-3">
          ...
          <div>
            ...
            <textarea value={form.reason} onChange={(e) => setForm((f) => ({ ...f, reason: e.target.value }))} rows={2} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg" />
          </div>
        </div>
